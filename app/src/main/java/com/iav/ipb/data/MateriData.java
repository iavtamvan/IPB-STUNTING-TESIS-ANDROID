...
//            R.drawable.tan_malaka
    ...
    public static ArrayList<MateriModel> getMateri(){
        ...
        ArrayList<MateriModel> list = new ArrayList<>();
        for (int i = 0; i < materiNames.length; i++) {
            materiModel = new MateriModel();
            materiModel.setName(materiNames[i]);
            materiModel.setDetail(materiDetails[i]);
            materiModel.setPhoto(materiImages[i]);

            list.add(materiModel);
        }
        return list;
    }
